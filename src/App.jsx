...
const VIDEO_URLS = {
  ...
  'TR-06': 'https://www.youtube.com/watch?v=qMQwM1Fv8gM',  // Cross-Body Cable Extension (Jeff Nippard)
  ...
  'CO-11': 'https://www.youtube.com/watch?v=iCqpUv_7h1s',  // Stomach Vacuum (Jeff Nippard)
...
const LEVELS = [
  ...
  {level:2,name:'Building Momentum',xp:500},
  {level:3,name:'Getting Consistent',xp:1500},
  {level:4,name:'Habit Forming',xp:3500},
  {level:5,name:'Machine',xp:7000},
  {level:6,name:'Iron Regular',xp:12000},
  {level:7,name:'Dedicated',xp:20000},
  {level:8,name:'Relentless',xp:35000},
  {level:9,name:'Elite',xp:55000},
  {level:10,name:'Legendary',xp:80000},
];

const XP_REWARDS = { set:10, exercise:25, mandatory:100, bonus:50, rehab:75, cardio:50, pr:100, streak:25 };
...
const DEFAULT_SETTINGS = {
  ...
  recoveryWindow: 48,       // hours (SOD2 default)
  cardioWeeklyTarget: 5,    // sessions per week
  maxRPE: 9,                // cap RPE (avoid 10 per SOD2)
  zone2HRMin: 120,          // bpm
  zone2HRMax: 145,          // bpm
  trainingTime: 'morning',  // 'morning','afternoon','evening' (COMT preference)
...
const DEFAULT_NUTRITION_CONFIG = {
  ...
  weightUnit: 'lbs',
  waistUnit: 'in',
  smoothingMethod: 'macrofactor',  // 'macrofactor' | 'happyscale' | 'libra'
  libraSmoothingDays: 7,
};
...
/** MacroFactor-style EWMA (α=0.05, ~20-day window) */
function smoothMacroFactor(logs, alpha = 0.05) {
...
/** Happy Scale-style bidirectional smoothing */
...
/** Libra-style time-adaptive EMA */
function smoothLibra(logs, smoothingDays = 7) {
  if (logs.length === 0) return [];
...
function getSmoothedWeights(logs, config) {
  const method = config?.smoothingMethod || 'macrofactor';
  switch (method) {
    case 'happyscale': return smoothHappyScale(logs);
    case 'libra': return smoothLibra(logs, config?.libraSmoothingDays || 7);
    default: return smoothMacroFactor(logs);
  }
}
...
// --- Adaptive TDEE Algorithm (MacroFactor-style) ---
...
function updateExpenditure(bodyLogs, nutritionConfig) {
  ...
  const smoothed = smoothMacroFactor(logsWithCals);
...
const BODY_XP = { weightLog: 5, nutritionFull: 5, allFields: 10, streak7: 25, streak30: 100 };
...
function MultiMetricOverlay({ settings, nutritionConfig, allBodyLogs, bodyLogs }) {
  ...
  const allSeries = useMemo(() => {
    ...
    const smoothed = weightLogs.length >= 2 ? smoothMacroFactor(weightLogs) : [];
...
function WeeklyCheckInCard({ nutritionConfig, onUpdateNutritionConfig, settings, allBodyLogs }) {
  ...
  const checkInData = useMemo(() => {
    ...
    if (recentWeight.length >= 3) {
      const smoothed = smoothMacroFactor(recentWeight);
      ...
      <div style={{ marginBottom:'20px' }}>
        ...
        <p style={{ fontSize:'13px', color:T.text2, marginTop:'4px' }}>
          Zone 2 · <button onClick={() => goToSettings?.('recovery')} style={{ background:'none', border:'none',
            ...
            textUnderlineOffset:'3px' }}>{weeklyTarget}×/week</button> for DRD2 dopamine
        </p>
      </div>
...
const DEFAULT_SYSTEM_PROMPT = `You are a rehab-focused exercise coach. Be concise (<120 words unless asked for detail). Reference clinical context when relevant. Never recommend exercises that conflict with the patient's contraindications.`;

const DEFAULT_PATIENT_PROFILE = `PATIENT: M/late-20s/6'1"/185lb/25%BF→goal<15%
DX: 1)Anterior humeral glide L>R(no OH press/upright row,≤45° flare) 2)Scapular dyskinesia I/III(low/mid trap inhibited) 3)Bilat glute med inhibition(TFL dominant) 4)Rec fem/psoas contracture→APT 5)Forefoot overload(sesamoid fx hx,neuromas)
GENETICS: SOD2+/-(low antioxidant,48hr recovery,no failure training) DRD2 homo(low dopamine,exercise=medicine,needs gamification) COMT+/-(no fasted training,AM exercise)
CONTRAINDICATIONS: No BB OH press,upright rows,standard planks,nordic curls,deep step-ups,behind-neck moves,heavy BB back squat,sit-ups,plyometrics(acute)
SUBS: OH press→landmine press; planks→dead bugs/Pallof; nordics→seated leg curl`;
...
- Reference the patient profile if provided for rehab-specific insights
- Use the user's weight unit consistently`;
    ...
    <div style={{ animation:'fadeIn 0.3s ease-out', paddingBottom:'40px' }}>
      ...
      <SettingsSection id="training" title="Training Profile" icon={Dumbbell} sectionRef={trainingRef}
        ...
        <div style={S.sub}>
          ...
          <div style={{ fontSize:'10px', color:T.text3, marginTop:'6px' }}>
            COMT +/- → AM exercise recommended for dopamine optimization.
          </div>
        </div>
      </SettingsSection>
      ...
      <SettingsSection id="recovery" title="Recovery & Programming" icon={Heart} sectionRef={recoveryRef}
        ...
        <div style={{ fontSize:'10px', color:T.text3, marginBottom:'8px', marginTop:'-4px' }}>
          SOD2 +/- → 48hr minimum recommended. Controls muscle map color thresholds.
        </div>
        <div style={S.row}>
        ...
        <div style={{ fontSize:'10px', color:T.text3, marginBottom:'8px', marginTop:'-4px' }}>
          SOD2 +/- → Avoid RPE 10 / training to failure. Default cap: 9.
        </div>
        <div style={S.row}>
      ...
      <SettingsSection id="bodyTracking" title="Body Tracking" icon={Scale} sectionRef={bodyTrackingRef}
        ...
        <div style={S.sub}>
          ...
          {[
            { key:'macrofactor', label:'MacroFactor (20-day EWMA)', desc:'Best for TDEE calculation' },
            { key:'happyscale', label:'Happy Scale (bidirectional)', desc:'Psychologically motivating' },
            { key:'libra', label:'Libra (time-adaptive)', desc:'Great for irregular weigh-ins' },
          ].map(m => (
          ...
          {nutritionConfig.smoothingMethod === 'libra' && (
            <div style={{ display:'flex', alignItems:'center', gap:'10px', padding:'8px 0 0 28px' }}>
              ...
              <input type="range" min={2} max={90} value={nutritionConfig.libraSmoothingDays || 7}
                onChange={e => onUpdateNutritionConfig({ ...nutritionConfig, libraSmoothingDays: Number(e.target.value) })}
                style={{ flex:1, accentColor:T.accent }} />
              <span style={S.val}>{nutritionConfig.libraSmoothingDays || 7}</span>
            </div>
          )}
        </div>
      ...
      <SettingsSection id="calibration" title="Body Composition Calibration" icon={Target} sectionRef={calibrationRef}
        ...
        <div style={S.sub}>
          ...
          <input type="text" value={calibration.deviceId || ''} placeholder="e.g. RunStar 8E"
      ...
      <div style={{ textAlign:'center', marginTop:'20px', fontSize:'11px', color:T.text3 }}>
        NeuroRehab v0.4 · Data stored locally
      </div>
    </div>
  );
}
